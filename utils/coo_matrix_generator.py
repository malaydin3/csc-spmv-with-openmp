...
if __name__ == "__main__":
    ...
    # Prefix for outputting files
    output_prefix = "../data/input/"

    # Vector 
    vector = np.random.rand(problem_size)
    np.savetxt(output_prefix + "vec.txt", vector, fmt='%.6f')
    ...
    save_coo_to_files(sparse_matrix,
                      output_prefix + 'row_indices.txt',
                      output_prefix + 'col_indices.txt',
                      output_prefix + 'values.txt',
                      output_prefix + 'dims.txt')
